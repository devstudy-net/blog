...
public final class ServiceManager {
    ...
    public String getApplicationProperty(String property) {
        ...
        if (value.startsWith(SYS_ENV)) {
            return getEnvironmentVariable(value.replace(SYS_ENV, "").replace("}", ""));
    ...
    private String getEnvironmentVariable(String value) {
        String result = System.getProperty(value);
        if (result != null) {
            return result;
        }
        result = System.getenv(value);
        if (result != null) {
            return result;
        }
        throw new IllegalArgumentException("Environment variable '" + value + "' not defined");
    }
